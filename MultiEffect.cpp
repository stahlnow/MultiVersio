...
const char *modes[NUM_MODES] = {"Reverb",
                                "Resonator",
                                "Filter",
                                "LO-FI",
                                "MicroLooper",
...
const float delay_times[NUM_DELAY_TIMES] = {0.0078125,
                                            0.015625,
                                            0.03125,
                                            0.25 / 6.f,
                                            0.046875,
...
/*
 * void GetFilterSamples(float  outl[],
 *                       float  outr[],
 *                       float  inl[],
 *                       float  inr[],
 *                       size_t size);
...
class LedsControl
{
  ...
  public:
    ...
    void UpdateLeds()
    {
        ...
        {
            versio.SetLed(
                i, base_color[i][0], base_color[i][1], base_color[i][2]);
            if(times[i] > 0)
            {
                times[i]--;
                versio.SetLed(
                    i, flash_color[i][0], flash_color[i][1], flash_color[i][2]);
            }
        }
        versio.UpdateLeds();
    };
...
class OscBank
{
  ...
  public:
    ...
    float Process()
    {
        ...
        {
            output_mult
                = ((0.5 + 0.2 / num_active) + prev_output_mult * 47.f) / 48.f;
            output
                = output
                  + osc[i].Process() * output_mult * attack_lut[attack_step[i]];
    ...
    void CalculateSpectralAnalisys()
    {
        ...
        for(int i = 0; i < spectra_max_num_frequencies; i++)
        {
            ...
            {
                freq[i] = findClosest(CHRM_SCALE,
                                      spectra_selected_scale,
                                      128,
                                      ((int)freq[i] * 1000),
                                      spectra_transpose)
                          / 1000.f;
            ...
            magn[i] = ((magni_fftoutbuff[a] / max_amp)
                           * (1 - spectra_lower_harmonics)
    ...
    void RemoveNearestBands(float frequency, size_t start_band)
    {
        ...
        for(size_t i = start_band;
            ...
            i++)
        {
            float mult           = map((i * bandSize / spectra_spread),
                             (1 * bandSize / spectra_spread),
                             upper_frequency,
                             0.0f,
                             1.0f);
                          ...
                          size_t                    size)
{
    ...
    {
        spectra_oscbank.FillInputBuffer(
            const_cast<float *>(in[0]), const_cast<float *>(in[1]), size);
    ...
    for(size_t i = 0; i < size; i += 1)
    {
        ...
        switch(mode)
        {
            ...
            case SPECTRA:
                ...
                break;
            case DELAY: GetDelaySample(out1, out2, in1, in2); break;
            case SPECTRINGS:
    ...
    if(mode == FILTER)
    {
        GetFilterSamples(out[0],
                         out[1],
                         const_cast<float *>(in[0]),
                         const_cast<float *>(in[1]),
                         size);
    }
};
...
int main(void)
{
    ...
    filter_cutoff_l_par.Init(versio.knobs[DaisyVersio::KNOB_0],
                             60,
                             20000,
                             filter_cutoff_l_par.LOGARITHMIC);
    filter_cutoff_r_par.Init(versio.knobs[DaisyVersio::KNOB_4],
                             60,
                             20000,
                             filter_cutoff_r_par.LOGARITHMIC);

    delay_cutoff_par.Init(versio.knobs[DaisyVersio::KNOB_1],
                          400,
                          20000,
    ...
    lofi_tone_par.Init(versio.knobs[DaisyVersio::KNOB_2],
                       20,
                       20000,
                       lofi_tone_par.LOGARITHMIC);
    lofi_rate_par.Init(versio.knobs[DaisyVersio::KNOB_1],
                       sample_rate * 4,
                       sample_rate / 16,
                       lofi_rate_par.LOGARITHMIC);
...

{
    ...
    switch(mode)
    {
        case REV:
            ...
            reverb_feedback
                = 0.8f + (std::log10(10 + regen * 90) - 1.000001f) * 0.4f;
        ...
        case RESONATOR:
            ...
            resonator_feedback
                = (std::log10(
                       10
                       + clamp((resonator_current_regen - 0.5) * 2.f, 0, 1)
                             * 90)
                   - 1.000001f)
                      * 1.5f
                  - (std::log10(
                         10
                         + clamp((1 - resonator_current_regen * 2.f), 0, 1)
                               * 90)
                     - 1.000001f)
                        * 1.5f;
        ...
        case LOFI:
            ...
            rev.SetLpFreq(reverb_lowpass);
            reverb_feedback
                = 0.7f + (std::log10(10 + regen * 90) - 1.000001f) * 0.3f;
            rev.SetFeedback(reverb_feedback);

            lofi_lpg_amount = size * size * 3;
            lofi_lpg_decay  = clamp(
                (1 - ((std::log10(10 + dense * 90) - 0.999991f) * 0.4f + 0.6f))
                    * 0.05f,
                0.0001,
                0.99999);
            //Shimmer
        ...
        case SPECTRA:
            ...
            spectra_num_active
                = ((int)(1
                         + (clamp(size * 2, 0.0f, 1.0f)
                            * (spectra_max_num_frequencies - 0.5f))));


            spectra_spread
                = clamp(map(size - 0.5, 0.0f, 0.5f, 1.0f, 4.0f), 1.0f, 4.0f);
            ...
            reverb_shimmer = 0.0f;
            reverb_feedback
                = 0.2f + (std::log10(10 + regen * 90) - 1.000001f) * 1.0f;
            rev.SetFeedback(reverb_feedback);
        ...
        case DELAY:
            ...
            if(delay_control_counter == 0)
            {
                ...
                if(delay_time_trig > 0)
                {
                    ...
                    delay_time[delay_inactive]
                        = (delay_time_trig * 4) % LOOPER_MAX_SIZE;
                ...
                if(index > 0.5f)
                {
                    if(!delay_frozen)
                    {
                        ...
                        delay_frozen_start
                            = (delay_write_pos - delay_time[delay_active]
                               + LOOPER_MAX_SIZE)
                              % LOOPER_MAX_SIZE;
            ...
            reverb_lowpass
                = (global_sample_rate + global_sample_rate * (size)) / 4.f;
            rev.SetLpFreq(reverb_lowpass);
            reverb_feedback
                = 0.65f + (std::log10(10 + size * 90) - 1.000001f) * 0.20f;
            rev.SetFeedback(reverb_feedback);
            //Shimmer
            reverb_shimmer     = 0.0f;
        ...
        case SPECTRINGS:
            ...
            for(int i = 0; i < NUM_OF_STRINGS; i++)
            {
                string_voice[i].SetBrightness(spectra_oscbank.getMagnitudo(i)
                                              * regen);
            ...
            reverb_lowpass
                = global_sample_rate * 0.4 * (1 - speed * speed * 0.6) / 2.f;
            rev.SetLpFreq(reverb_lowpass);
            reverb_shimmer = 0.0f;
            reverb_feedback
            ...
            reverb_drywet = clamp(map(clamp(speed * 1.1f, 0.0f, 1.0f) * 0.95,
                                      0.0,
                                      0.95,
                                      0.7f,
                                      0.95f),
...
float CompressSample(float sample)
{
    ...
    {
        sample
            = clamp(sample - map(sample, 0.4f, 5.0f, 0.0f, 0.6f), 0.0f, 2.0f);
    }
    if(sample < -0.4)
    {
        sample = clamp(
            sample - map(sample, -5.0f, -0.4f, -0.6f, 0.0f), -2.0f, 0.0f);
    }

    if(sample > 0.8)
    {
        sample
            = clamp(sample - map(sample, 0.8f, 2.0f, 0.0f, 0.1f), 0.0f, 0.9f);
    }
    if(sample < -0.8)
    {
        sample = clamp(
            sample - map(sample, -2.0f, -0.8f, -0.1f, 0.0f), -0.9f, 0.0f);
    }
    return sample;
}
...
void GetReverbSample(float &outl, float &outr, float inl, float inr)
{
    ...
    {
        float octave2_shim
            = mlooper_frozen_buf_1l[(int)reverb_shimmer_play_pos2];
        shimmer_l                = shimmer_l + octave2_shim;
        shimmer_r                = shimmer_r + octave2_shim;
    ...
    {
        leds.SetBaseColor(0,
                          clamp(reverb_current_RMS, 0, 1),
                          clamp(reverb_target_RMS, 0, 1),
                          clamp(reverb_current_RMS, 0, 1)
                              * clamp(reverb_current_RMS, 0, 1));
        leds.SetBaseColor(1,
                          clamp(reverb_target_RMS, 0, 1),
    ...
    float sum_inl
        = (inl
           + shimmer_l * reverb_shimmer * (reverb_feedback * 0.5f + 0.5f)
                 * (0.5f + reverb_current_RMS * 0.5f))
          * 0.5f;
    float sum_inr
        = (inr
           + shimmer_r * reverb_shimmer * (reverb_feedback * 0.5f + 0.5f)
                 * (0.5f + reverb_current_RMS * 0.5f))
          * 0.5f;
...
void GetResonatorSample(float &outl, float &outr, float inl, float inr)
{
    ...
    fonepole(resonator_current_delay,
...
void GetLofiSample(float &outl, float &outr, float inl, float inr)
{
    ...
    {
        fonepole(
            lofi_current_RMS, lofi_target_RMS, .005f * lofi_lpg_decay * 10.f);
    }
    else
    {
    ...
    fonepole(lofi_current_Lofi_LFO_Freq,
             lofi_target_Lofi_LFO_Freq,
    ...
    float lofi_left = lofi_leftFilter
                      + (((200.f - clamp(lofi_cutoff, 20.f, 200.f)) / 200.f)
                         * lofi_rightFilter)
                      + lofi_leftFilter * lofi_variable_compressor
                      + lofi_leftFilter * lofi_variable_compressor
                            * lofi_current_left_saturation * 0.01f;
    float lofi_right = lofi_rightFilter
                       + (((200.f - clamp(lofi_cutoff, 20.f, 200.f)) / 200.f)
                          * lofi_leftFilter)
    ...
    {
        lofi_left = clamp(
            lofi_left - map(lofi_left, 0.4f, 10.0f, 0.0f, 0.6f), 0.0f, 1.0f);
    }
    if(lofi_right > 0.4)
    {
        lofi_right = clamp(
            lofi_right - map(lofi_right, 0.4f, 10.0f, 0.0f, 0.6f), 0.0f, 1.0f);
    }

    if(lofi_left < -0.4)
    {
        lofi_left
            = clamp(lofi_left - map(lofi_left, -10.0f, -0.4f, -0.6f, 0.0f),
                    -1.0f,
                    0.0f);
    }
    if(lofi_right < -0.4)
    {
        lofi_right
            = clamp(lofi_right - map(lofi_right, -10.0f, -0.4f, -0.6f, 0.0f),
                    -1.0f,
                    0.0f);
    }
...
{ //writing the incoming input into the buffer
    ...
    if(mlooper_play)
    {
        if(!mlooper_frozen) //if the looper is not frozen
        {
            ...
            if(mlooper_play)
            {
                ...
                modified_buffer_length_l
                    = (int)(mlooper_len * mlooper_division_1);
                if(mlooper_pos_1 > modified_buffer_length_l)
                {
                    leds.SetForXCycles(0, 10, 1, 0, 0);
                    mlooper_pos_1
                        = clamp(mlooper_pos_1 - modified_buffer_length_l,
                                0,
                                mlooper_len);
                }
                ...
                modified_buffer_length_r
                    = (int)(mlooper_len * mlooper_division_2);
                if(mlooper_pos_2 > modified_buffer_length_r)
                {
                    leds.SetForXCycles(3, 10, 1, 0, 0);
                    mlooper_pos_2
                        = clamp(mlooper_pos_2 - modified_buffer_length_r,
                                0,
                                mlooper_len);
                }
        ...
        { //if the buffer is not frozen we get one sample from the frozen looper
            ...
            if(mlooper_play)
            {
                ...
                mlooper_frozen_pos_1
                    = mlooper_frozen_pos_1 + mlooper_play_speed_1;
                ...
                if(mlooper_frozen_pos_1 > modified_frozen_buffer_length_l)
                {
                    ...
                    mlooper_frozen_pos_1 = clamp(
                        mlooper_frozen_pos_1 - modified_frozen_buffer_length_l,
                        0,
                        mlooper_len);
                }
                ...
                mlooper_frozen_pos_2
                    = mlooper_frozen_pos_2 + mlooper_play_speed_2;
                ...
                if(mlooper_frozen_pos_2 > modified_frozen_buffer_length_r)
                {
                    ...
                    mlooper_frozen_pos_2 = clamp(
                        mlooper_frozen_pos_2 - modified_frozen_buffer_length_r,
                        0,
                        mlooper_len);
                }
                ...
                out1l = GetSampleFromBuffer(mlooper_frozen_buf_1l,
                                            mlooper_frozen_pos_1)
                        * mlooper_volume_att_1;
                out1r = GetSampleFromBuffer(mlooper_frozen_buf_1r,
                                            mlooper_frozen_pos_2)
                        * mlooper_volume_att_2;
            }
        };
    ...
    mlooper_writer_outside_pos
        = ((mlooper_writer_outside_pos + mlooper_len_count)
           % (mlooper_len_count))
          % LOOPER_MAX_SIZE;
...
void GetDelaySample(float &out1l, float &out1r, float in1l, float in1r)
{
    ...
    fonepole(delay_feedback_RMS,
             delay_target_RMS,
             .001f * (1 / (0.5 + delay_feedback_RMS)));

    fonepole(delay_fast_feedback_RMS,
             delay_target_RMS,
    ...
    if(delay_time[delay_inactive] > 0)
    {
        ...
        {
            delay_xfade_current = clamp(
                delay_xfade_current - (1.f / (47.f * delay_control_latency_ms)),
                0,
                1);
        }
...
void GetSpectraSample(float &outl, float &outr, float inl, float inr)
{
    ...
    spectra_output
        = spectra_output; //(sqrt(0.5f * (spectra_dynamics*2.0f))*filtered_out + sqrt(0.95f * (2.f - (spectra_dynamics*2))) * spectra_output)*0.5f;
