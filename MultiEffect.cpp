...
// NATURAL GATE: LPG with SPECTRAL ANALISYS FOR NOTE HEIGHT, NOTE REPETITION
// DISTANCE CLOCKED DELAY NORMALE
...
const char *modes[NUM_MODES] = {"Reverb",  "Resonator",   "Filter",
                                "LO-FI",   "MicroLooper", "Delay",
                                "Spectra", "Spectrings",  "Natural Gate"};
...
const float delay_times[NUM_DELAY_TIMES] = {
    0.0078125,  0.015625, 0.03125, 0.25 / 6.f, 0.046875, 0.0625,
    0.25 / 3.f, 0.09375,  0.125,   0.5 / 3.f,  0.1875,   0.25,
    1.f / 3.f,  0.375,    0.5f,    0.75f,      1.f};
...
int CHRM_SCALE[128] = {
    ...
    8372018, 8869844, 9397273, 9956063, 10548080, 11175300, 11839820, 12543850};
...
float resonator_current_delay, resonator_feedback, resonator_target,
...
float filter_target_l_freq, filter_target_r_freq, filter_current_l_freq,
    filter_current_r_freq = 0.5f;
...
void GetFilterSamples(float *outl, float *outr, float *inl, float *inr,
                      size_t size);
...
public:
  ...
  void SwitchAllOff() { SetAllLeds(0, 0, 0); };
  void SwitchOffLed(int idx) { versio.SetLed(idx, 0, 0, 0); };
  ...
  void UpdateLeds() {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (times[i] > 0) {
        times[i]--;
        versio.SetLed(i, flash_color[i][0], flash_color[i][1],
                      flash_color[i][2]);
      }
    }
    versio.UpdateLeds();
...
public:
  ...
  void SetFreq(int index, float frequency) { osc[index].SetFreq(frequency); };
  void SetAmp(int index, float amplitude) { osc[index].SetAmp(amplitude); };
  float SetAllWaveforms(int waveform) {
    ...
    for (int i = 0; i < number_of_osc; i++) {
      // the second time it actually changes the waveform, so the attack lut can
      // avoid clicks
  ...
  float Process() {
    ...
    for (int i = 0; i < spectra_max_num_frequencies; i++) {
      ...
      output =
          output + osc[i].Process() * output_mult * attack_lut[attack_step[i]];
      attack_step[i] = clamp(attack_step[i] + 1, 0, 299);
      prev_output_mult = output_mult;
    };
    return output;
  }
  size_t GetPasses() { return num_of_passes; }
  ...
  void CalculateSpectralAnalisys() {
    ...
    for (int i = 0; i < spectra_max_num_frequencies; i++) {
      int a = std::distance(
          magni_fftoutbuff,
          std::max_element(magni_fftoutbuff, magni_fftoutbuff + (N / 2)));

      freq[i] = a * bandSize * spectra_oct_mult;
      if (spectra_quantize > 0) {
        freq[i] = findClosest(CHRM_SCALE, spectra_selected_scale, 128,
                              ((int)freq[i] * 1000), spectra_transpose) /
                  1000.f;
      };
      max_amp = std::max(magni_fftoutbuff[a], max_amp);
      magn[i] =
          ((magni_fftoutbuff[a] / max_amp) * (1 - spectra_lower_harmonics) +
           spectra_lower_harmonics);
  ...
  void RemoveNearestBands(float frequency, size_t start_band) {
    ...
    float upper_frequency =
        mtof((int)((12.f * log2(frequency / 440.f) + 69 + 1)));
    for (size_t i = start_band;
         (((i * bandSize / spectra_spread) < upper_frequency) &
          (i < FFT_SIZE / 2) & (-i > 0));
         i++) {
      float mult =
          map((i * bandSize / spectra_spread), (1 * bandSize / spectra_spread),
              upper_frequency, 0.0f, 1.0f);
      magni_fftoutbuff[i] = magni_fftoutbuff[i] * mult;
      magni_fftoutbuff[-i] = magni_fftoutbuff[-i] * mult;
    }
  }
  float getFrequency(int value) { return current_freq[value]; }
  float getMagnitudo(int value) { return current_magn[value]; }
  ...
  void SetNumActive(int value) { num_active = value; };
};
...
static void AudioCallback(AudioHandle::InputBuffer in,
  ...
  if ((mode == SPECTRA) or (mode == SPECTRINGS)) {
    spectra_oscbank.FillInputBuffer(const_cast<float *>(in[0]),
                                    const_cast<float *>(in[1]), size);
    ...
    if (mode == SPECTRINGS) {
      string_voice[spectrings_current_voice].SetFreq(
          spectra_oscbank.getFrequency(spectrings_current_voice));
    }
  };
  ...
  for (size_t i = 0; i < size; i += 1) {
    ...
    case REV:
      ...
      break;
    case RESONATOR:
      GetResonatorSample(out1, out2, in1, in2);
      break;
    case LOFI:
      GetReverbSample(out1, out2, in1, in2);
      ...
      break;
    case MLOOPER:
      GetLooperSample(out1, out2, in1, in2);
      break;
    case SPECTRA:
      GetSpectraSample(out1, out2, in1, in2);
      GetReverbSample(out1, out2, out1, out2);
      break;
    case DELAY:
      GetDelaySample(out1, out2, in1, in2);
      break;
    case SPECTRINGS:
      GetSpectringsSample(out1, out2, in1, in2);
      GetReverbSample(out1, out2, out1, out2);
      break;
  ...
  if (mode == FILTER) {
    GetFilterSamples(out[0], out[1], const_cast<float *>(in[0]),
                     const_cast<float *>(in[1]), size);
  }
};
...
int main(void) {
  ...
  // crusher_cutoff_par.Init(versio.knobs[DaisyVersio::KNOB_0], 60, 20000,
  // crusher_cutoff_par.LOGARITHMIC);
  // crusher_crushrate_par.Init(versio.knobs[DaisyVersio::KNOB_1], 1, 50,
  // crusher_crushrate_par.LOGARITHMIC);
  filter_cutoff_l_par.Init(versio.knobs[DaisyVersio::KNOB_0], 60, 20000,
                           filter_cutoff_l_par.LOGARITHMIC);
  filter_cutoff_r_par.Init(versio.knobs[DaisyVersio::KNOB_4], 60, 20000,
                           filter_cutoff_r_par.LOGARITHMIC);

  delay_cutoff_par.Init(versio.knobs[DaisyVersio::KNOB_1], 400, 20000,
                        delay_cutoff_par.LOGARITHMIC);
  ...
  lofi_tone_par.Init(versio.knobs[DaisyVersio::KNOB_2], 20, 20000,
                     lofi_tone_par.LOGARITHMIC);
  lofi_rate_par.Init(versio.knobs[DaisyVersio::KNOB_1], sample_rate * 4,
                     sample_rate / 16, lofi_rate_par.LOGARITHMIC);
...

{
  ...
  case RESONATOR:
    ...
    if (versio.tap.RisingEdge()) {
      ...
      resonator_glide =
          resonator_glide_mode * resonator_glide_mode * resonator_glide_mode;
    };
  ...
  case FILTER:
    ...
    svf2l.set_f_q<stmlib::FREQUENCY_ACCURATE>(filter_current_l_freq,
                                              1.f + speed * speed * 49.f);
    svf2r.set_f_q<stmlib::FREQUENCY_ACCURATE>(filter_current_r_freq,
                                              1.f + size * size * 49.f);
  ...
  case LOFI:
    ...
    lofi_lpg_decay = clamp(
        (1 - ((std::log10(10 + dense * 90) - 0.999991f) * 0.4f + 0.6f)) * 0.05f,
        0.0001, 0.99999);
    // Shimmer
    reverb_shimmer = 0.0f;
  ...
  case SPECTRA:
    ...
    spectra_prev_knob_wave_knob = spectra_oscbank.SetAllWaveforms(
        (int)((dense * 9 + spectra_prev_knob_wave_knob) * 0.1 * 9.f));

    spectra_num_active = ((int)(1 + (clamp(size * 2, 0.0f, 1.0f) *
                                     (spectra_max_num_frequencies - 0.5f))));
  ...
  case DELAY:
    ...
    // Change delay length only if the difference is higher than 0.5
    // milliseconds to avoid clicks for micro changes in the delay time
    if (delay_control_counter == 0) {
      ...
      if (delay_time_trig > 0) {
        // if (abs(delay_time[delay_active] - ((delay_time_count*4) %
        // LOOPER_MAX_SIZE)) > 48)
        ...
        if (delay_main_counter == 0) {
          delay_left_counter =
              (delay_write_pos - delay_pos_l[delay_active]) / 4;
          delay_right_counter =
              (delay_write_pos - delay_pos_r[delay_active]) / 4;
      ...
      if (index > 0.5f) {
        if (!delay_frozen) {
          ...
          delay_frozen_start =
              (delay_write_pos - delay_time[delay_active] + LOOPER_MAX_SIZE) %
              LOOPER_MAX_SIZE;
    ...
    delay_control_counter =
        (delay_control_counter + 1) % delay_control_latency_ms;

    // SelectLooperPlaySpeed(speed,size);
    delay_feedback =
        size * 0.1 + (std::log10(10 + tone * 90) - 1.000001f) * 0.9f;
  ...
  case SPECTRINGS:
    ...
    if (versio.gate.Trig()) {
      ...
      spectrings_accent_amount[spectrings_current_voice] =
          spectra_oscbank.getMagnitudo(spectrings_current_voice);
...
void WriteShimmerBuffer1(float in_l, float in_r) {
  ...
  reverb_shimmer_write_pos1l =
      (reverb_shimmer_write_pos1l + 1) % reverb_shimmer_buffer_size1l;
  reverb_shimmer_write_pos1r =
      (reverb_shimmer_write_pos1r + 1) % reverb_shimmer_buffer_size1r;
};

void WriteShimmerBuffer2(float in_l, float in_r) {
  // writes the input to the buffer
  mlooper_frozen_buf_1l[reverb_shimmer_write_pos2] = (in_r + in_l) / 2;
  reverb_shimmer_write_pos2 =
      (reverb_shimmer_write_pos2 + 1) % reverb_shimmer_buffer_size2;
};
float CompressSample(float sample) {
  ...
  if (sample < -0.4) {
    sample =
        clamp(sample - map(sample, -5.0f, -0.4f, -0.6f, 0.0f), -2.0f, 0.0f);
  }
  ...
  if (sample < -0.8) {
    sample =
        clamp(sample - map(sample, -2.0f, -0.8f, -0.1f, 0.0f), -0.9f, 0.0f);
  }
  return sample;
}

void GetReverbSample(float &outl, float &outr, float inl, float inr) {
  // Shimmer part: basically we write the buffer once every two frames and then
  // we read it every frame at two different speeds so two octaves are produced
  // (the higher one is reduced in intensity)
  ...
  if (((mode == REV)) or (mode == RESONATOR)) {
    ...
    reverb_shimmer_play_pos1l =
        (reverb_shimmer_play_pos1l + 2) % reverb_shimmer_buffer_size1l;
    reverb_shimmer_play_pos1r =
        (reverb_shimmer_play_pos1r + 2) % reverb_shimmer_buffer_size1r;
    WriteShimmerBuffer1(inl, inr);
  }
  if (mode == REV) {
    float octave2_shim =
        mlooper_frozen_buf_1l[(int)reverb_shimmer_play_pos2] * 0.5f;
  ...
  if (mode == REV) {
    leds.SetBaseColor(
        0, clamp(reverb_current_RMS, 0, 1), clamp(reverb_target_RMS, 0, 1),
        clamp(reverb_current_RMS, 0, 1) * clamp(reverb_current_RMS, 0, 1));
    leds.SetBaseColor(
        1, clamp(reverb_target_RMS, 0, 1), clamp(reverb_target_RMS, 0, 1),
        clamp(reverb_target_RMS, 0, 1) * clamp(reverb_target_RMS, 0, 1));
    leds.SetBaseColor(
        3, clamp(reverb_current_RMS, 0, 1), clamp(reverb_target_RMS, 0, 1),
        clamp(reverb_current_RMS, 0, 1) * clamp(reverb_current_RMS, 0, 1));
    leds.SetBaseColor(
        2, clamp(reverb_target_RMS, 0, 1), clamp(reverb_target_RMS, 0, 1),
        clamp(reverb_target_RMS, 0, 1) * clamp(reverb_target_RMS, 0, 1));
  }

  rev.SetFeedback(reverb_feedback - reverb_feedback_RMS * 0.75f);
  // summing the output of the incoming audio, the previous input, and the
  // shimmer
  float sum_inl =
      (inl + shimmer_l * reverb_shimmer * (reverb_feedback * 0.5f + 0.5f) *
                 (0.5f + reverb_current_RMS * 0.5f)) *
      0.5f;
  float sum_inr =
      (inr + shimmer_r * reverb_shimmer * (reverb_feedback * 0.5f + 0.5f) *
                 (0.5f + reverb_current_RMS * 0.5f)) *
      0.5f;
  ...
  reverb_averager.Add((reverb_current_outl * reverb_current_outl +
                       reverb_current_outr * reverb_current_outr) /
                      2);
...
void GetResonatorSample(float &outl, float &outr, float inl, float inr) {
  ...
  fonepole(resonator_current_delay, resonator_target / resonator_octave,
  ...
  leds.SetBaseColor(0, clamp(resonator_current_RMS, 0, 1),
                    clamp(resonator_current_RMS, 0, 1) *
                        clamp(resonator_current_RMS, 0, 0.1),
                    (resonator_glide_mode / 10.f));
  leds.SetBaseColor(1, clamp(resonator_feedback_RMS, 0, 1),
                    clamp(resonator_feedback_RMS, 0, 1) *
                        clamp(resonator_feedback_RMS, 0, 0.1),
                    (resonator_glide_mode / 10.f));
  leds.SetBaseColor(3, clamp(resonator_current_RMS, 0, 1),
                    clamp(resonator_current_RMS, 0, 1) *
                        clamp(resonator_current_RMS, 0, 0.1),
                    (resonator_glide_mode / 10.f));
  leds.SetBaseColor(2, clamp(resonator_feedback_RMS, 0, 1),
                    clamp(resonator_feedback_RMS, 0, 1) *
                        clamp(resonator_feedback_RMS, 0, 0.1),
                    (resonator_glide_mode / 10.f));

  // Setting the reverb feedback to scale according to the intensity of the
  // audio rev.SetFeedback(resonator_feedback
  // -resonator_feedback_RMS*0.75f*(2.f-resonator_feedback));
  ...
  resonator_averager.Add(
      (resonator_outl * resonator_outl + resonator_outr * resonator_outr) / 2);

  float delay_input_l = 0.f;
  float delay_input_r = 0.f;
  if (resonator_feedback > 0) {
    delay_input_l = dcblock_l.Process((
        (resonator_feedback - resonator_current_RMS * 0.85f) *
        (resonator_outl + rev_outl * (0.15 + 0.85f * (1 - resonator_drywet)))));
    delay_input_r = dcblock_r.Process((
        (resonator_feedback - resonator_current_RMS * 0.85f) *
        (resonator_outr + rev_outr * (0.15 + 0.85f * (1 - resonator_drywet)))));
  } else {
    delay_input_l = dcblock_l.Process((
        (resonator_feedback + resonator_current_RMS * 0.85f) *
        (resonator_outl + rev_outl * (0.15 + 0.85f * (1 - resonator_drywet)))));
...
void GetFilterSamples(float *outl, float *outr, float *inl, float *inr,
  ...
  for (size_t i = 0; i < size; i++) {
    inr[i] = sqrt(0.5f * (clamp((filter_path - 0.05f), 0.0f, 1.f) * 2.0f)) *
                 outl[i] +
...
void GetLofiSample(float &outl, float &outr, float inl, float inr) {
  inl = inl * 0.8f;
  inr = inr * 0.8f;
  // RMS calculation with smoothing for the envelope follower and the variable
  // compressor. RMS is calculated every RMS_SIZE samples
  lofi_rmsCount++;
  ...
  float lofi_envelope_follower =
      clamp(lofi_current_RMS * lofi_cutoff * 13.0f, 20.f, 20000.f);
  ...
  svf2l.set_f_q<stmlib::FREQUENCY_FAST>(
      lofi_envelope_follower / global_sample_rate, 1.f);
  svf2r.set_f_q<stmlib::FREQUENCY_FAST>(
      lofi_envelope_follower / global_sample_rate, 1.f);
  // when knob1 is very low an hi-pass filter activates to create a more distant
  // sound
  float lofi_hipass_freq = clamp(200.0 - (lofi_cutoff * 2), 0.0f, 200);
  svfl.SetFreq(lofi_hipass_freq);
  svfr.SetFreq(lofi_hipass_freq);

  // knob 2 sets how often the delay modulation changes. This time is variable
  // between 0 and lofi_mod time.
  ...
  // This smoothing allows for the delay time to change slowly so the pitch
  // shifting effect is subtle
  fonepole(lofi_current_Lofi_LFO_Freq, lofi_target_Lofi_LFO_Freq,
           1.0 / (1.2f * (lofi_damp_speed + (lofi_mod * 3) / 2)));
  ...
  outl = sqrt(0.5f * (lofi_drywet * 2.0f)) * dell.Read() +
         sqrt(0.95f * (2.f - (lofi_drywet * 2))) * inl;
  outr = sqrt(0.5f * (lofi_drywet * 2.0f)) * delr.Read() +
         sqrt(0.95f * (2.f - (lofi_drywet * 2))) * inr;
  ...
  // they work a bit differently. The Tone objects accepts the input as a
  // parameter and gives back a filtered sample. The SVF accepts an input and we
  // retrive the hi-pass ouput with High
  ...
  // Here we calculate a basic compressor using the RMS values of the envelope
  // follower this way we compensate for the lack of volume when filtering by
  // interpolating it with the previous state of the compressor, we slightly
  // smooth the change avoiding potential clicks.
  float lofi_variable_compressor =
      (2.f * lofi_previous_variable_compressor +
       ((300.f - clamp(lofi_cutoff, 30.f, 300.f)) / 300.f) *
           (1.f - lofi_current_RMS) * 1.f) *
      0.33f;
  lofi_previous_variable_compressor = lofi_variable_compressor;

  // This is a simple saturation that will contribute in enhancing the volume
  // "vintage way" when knob 1 is low
  float lofi_current_left_saturation = abs(inl * inl);
  lofi_previous_left_saturation =
      (lofi_current_left_saturation + lofi_previous_left_saturation * 9.f) /
      10.f;
  float lofi_current_right_saturation = abs(inr * inr);
  lofi_previous_right_saturation =
      (lofi_current_right_saturation + lofi_previous_right_saturation * 9.f) /
      10.f;

  // Here we calculate the outputs, which are the filtered waveform plus, a
  // certain amount of the other channel to "monoize it" when knob 1 is low,
  // plus a certain amount of compression and saturation.
  float lofi_left =
      lofi_leftFilter +
      (((200.f - clamp(lofi_cutoff, 20.f, 200.f)) / 200.f) * lofi_rightFilter) +
      lofi_leftFilter * lofi_variable_compressor +
      lofi_leftFilter * lofi_variable_compressor *
          lofi_current_left_saturation * 0.01f;
  float lofi_right =
      lofi_rightFilter +
      (((200.f - clamp(lofi_cutoff, 20.f, 200.f)) / 200.f) * lofi_leftFilter) +
      lofi_rightFilter * lofi_variable_compressor +
      lofi_rightFilter * lofi_variable_compressor *
          lofi_current_right_saturation * 0.01f;

  // we still add a certain amount of the other channel to further monoize the
  // sound
  lofi_left = lofi_left +
              lofi_right * ((200.f - clamp(lofi_cutoff, 20.f, 200.f)) / 200.f);
  lofi_right = lofi_right +
               lofi_left * ((200.f - clamp(lofi_cutoff, 20.f, 200.f)) / 200.f);

  // this is a basic instantaneous saturation/limiter: if the sound is too loud
  // (in either) directions, we compress it to avoid digital clipping.
  if (lofi_left > 0.4) {
    lofi_left =
        clamp(lofi_left - map(lofi_left, 0.4f, 10.0f, 0.0f, 0.6f), 0.0f, 1.0f);
  }
  if (lofi_right > 0.4) {
    lofi_right = clamp(lofi_right - map(lofi_right, 0.4f, 10.0f, 0.0f, 0.6f),
                       0.0f, 1.0f);
  }

  if (lofi_left < -0.4) {
    lofi_left = clamp(lofi_left - map(lofi_left, -10.0f, -0.4f, -0.6f, 0.0f),
                      -1.0f, 0.0f);
  }
  if (lofi_right < -0.4) {
    lofi_right = clamp(lofi_right - map(lofi_right, -10.0f, -0.4f, -0.6f, 0.0f),
                       -1.0f, 0.0f);
  }
...
void WriteLooperBuffer(float in_1l, float in_1r) {
  ...
  // this allows to fill the buffer when the next buffer is going to be bigger
...
float GetSampleFromBuffer(float buffer[], float pos) {
  // linear interpolation that gives back one sample in a certain position in
  // the buffer
...
void GetLooperSample(float &out1l, float &out1r, float in1l,
                     float in1r) { // writing the incoming input into the buffer
  out1l = out1r = 0;
  WriteLooperBuffer(in1l, in1r);
  // advance the buffer writing cursor and wrap it if it's longer than the
  // buffer length
  mlooper_writer_pos++;
  ...
  if (mlooper_play) {
    if (!mlooper_frozen) // if the looper is not frozen
    {
      ...
      if (mlooper_play) {
        // now we change the play_pos according to the number of repetitions and
        // the playing speed
        ...
        if (mlooper_pos_1 > modified_buffer_length_l) {
          ...
          mlooper_pos_1 =
              clamp(mlooper_pos_1 - modified_buffer_length_l, 0, mlooper_len);
        ...
        if (mlooper_pos_2 > modified_buffer_length_r) {
          ...
          mlooper_pos_2 =
              clamp(mlooper_pos_2 - modified_buffer_length_r, 0, mlooper_len);
        } else if (mlooper_pos_2 < 0.f) {
          mlooper_pos_2 =
              clamp(mlooper_pos_2 + modified_buffer_length_r, 0, mlooper_len);
        }

        out1l = GetSampleFromBuffer(mlooper_buf_1l, mlooper_pos_1) *
                mlooper_volume_att_1;
        out1r = GetSampleFromBuffer(mlooper_buf_1r, mlooper_pos_2) *
                mlooper_volume_att_2;
      };
    } else // Frozen Buffer
    { // if the buffer is not frozen we get one sample from the frozen looper

      // out1l =
      // GetSampleFromBuffer(mlooper_frozen_buf_1l,mlooper_frozen_pos_1)*mlooper_volume_att_1;
      // out1r =
      // GetSampleFromBuffer(mlooper_frozen_buf_1r,mlooper_frozen_pos_2)*mlooper_volume_att_2;

      if (mlooper_play) {
        // now we change the play_pos according to the number of repetitions and
        // the playing speed
        mlooper_frozen_pos_1 = mlooper_frozen_pos_1 + mlooper_play_speed_1;
        modified_frozen_buffer_length_l =
            (int)(mlooper_frozen_len * mlooper_division_1);
        if (mlooper_frozen_pos_1 > modified_frozen_buffer_length_l) {
          leds.SetForXCycles(0, 10, 0, 0, 1);
          mlooper_frozen_pos_1 =
              clamp(mlooper_frozen_pos_1 - modified_frozen_buffer_length_l, 0,
                    mlooper_len);
        ...
        modified_frozen_buffer_length_r =
            (int)(mlooper_frozen_len * mlooper_division_2);
        if (mlooper_frozen_pos_2 > modified_frozen_buffer_length_r) {
          leds.SetForXCycles(3, 10, 0, 0, 1);
          mlooper_frozen_pos_2 =
              clamp(mlooper_frozen_pos_2 - modified_frozen_buffer_length_r, 0,
                    mlooper_len);
        ...
        out1l =
            GetSampleFromBuffer(mlooper_frozen_buf_1l, mlooper_frozen_pos_1) *
            mlooper_volume_att_1;
        out1r =
            GetSampleFromBuffer(mlooper_frozen_buf_1r, mlooper_frozen_pos_2) *
            mlooper_volume_att_2;
      }
    };
  }
  ...
  mlooper_writer_outside_pos =
      ((mlooper_writer_outside_pos + mlooper_len_count) % (mlooper_len_count)) %
      LOOPER_MAX_SIZE;
  ...
  out1l = sqrt(0.5f * (mlooper_drywet * 2.0f)) * out1l +
          sqrt(0.95f * (2.f - (mlooper_drywet * 2))) * in1l;
  out1r = sqrt(0.5f * (mlooper_drywet * 2.0f)) * out1r +
          sqrt(0.95f * (2.f - (mlooper_drywet * 2))) * in1r;
};
...
void SelectDelayDivision(float knob1, float knob2) {
  ...
  if ((new_delay_mult_l != delay_mult_l[delay_active]) or
      (new_delay_mult_r != delay_mult_r[delay_active])) {
...
void GetDelaySample(float &out1l, float &out1r, float in1l, float in1r) {
  ...
  fonepole(delay_feedback_RMS, delay_target_RMS,
           .001f * (1 / (0.5 + delay_feedback_RMS)));

  fonepole(delay_fast_feedback_RMS, delay_target_RMS,
           .0005f * (1 / (0.7 + delay_fast_feedback_RMS)));

  float input_l =
      dcblock_2l.Process(delay_prev_sample_l * delay_feedback *
                             (1 - delay_feedback_RMS * 0.3) +
                         in1l * (clamp(1 - delay_feedback, 0.5, 1))) *
      (1 - delay_fast_feedback_RMS * 0.4);
  float input_r =
      dcblock_2r.Process(delay_prev_sample_r * delay_feedback *
                             (1 - delay_feedback_RMS * 0.3) +
                         in1r * (clamp(1 - delay_feedback, 0.5, 1))) *
      (1 - delay_fast_feedback_RMS * 0.4);
  ...
  if (delay_time[delay_inactive] > 0) {
    if (!delay_frozen) {
      delay_pos_l[0] =
          ((int)((delay_write_pos - (delay_time[0] * delay_mult_l[0])) +
                 LOOPER_MAX_SIZE)) %
          LOOPER_MAX_SIZE;
      delay_pos_r[0] =
          ((int)((delay_write_pos - (delay_time[0] * delay_mult_r[0])) +
                 LOOPER_MAX_SIZE)) %
          LOOPER_MAX_SIZE;
      ...
      delay_pos_l[1] =
          ((int)((delay_write_pos - (delay_time[1] * delay_mult_l[1])) +
                 LOOPER_MAX_SIZE)) %
          LOOPER_MAX_SIZE;
      delay_pos_r[1] =
          ((int)((delay_write_pos - (delay_time[1] * delay_mult_r[1])) +
                 LOOPER_MAX_SIZE)) %
          LOOPER_MAX_SIZE;
      ...
      delay_pos_l[0] =
          ((int)((delay_frozen_pos - (delay_time[0] * delay_mult_l[0])) +
                 LOOPER_MAX_SIZE)) %
          LOOPER_MAX_SIZE;
      delay_pos_r[0] =
          ((int)((delay_frozen_pos - (delay_time[0] * delay_mult_r[0])) +
                 LOOPER_MAX_SIZE)) %
          LOOPER_MAX_SIZE;
      ...
      delay_pos_l[1] =
          ((int)((delay_frozen_pos - (delay_time[1] * delay_mult_l[1])) +
                 LOOPER_MAX_SIZE)) %
          LOOPER_MAX_SIZE;
      delay_pos_r[1] =
          ((int)((delay_frozen_pos - (delay_time[1] * delay_mult_r[1])) +
                 LOOPER_MAX_SIZE)) %
          LOOPER_MAX_SIZE;
    ...
    if (delay_xfade_current > delay_xfade_target) {
      delay_xfade_current =
          clamp(delay_xfade_current - (1.f / (47.f * delay_control_latency_ms)),
                0, 1);
  ...
  delay_outputl =
      sqrt(0.5f * (delay_xfade_current * 2.0f)) * delay_outl[1] +
      sqrt(0.95f * (2.f - (delay_xfade_current * 2))) * delay_outl[0];
  delay_outputr =
      sqrt(0.5f * (delay_xfade_current * 2.0f)) * delay_outr[1] +
      sqrt(0.95f * (2.f - (delay_xfade_current * 2))) * delay_outr[0];
  ...
  // float filter_out1l =
  // svf2l.Process<stmlib::FILTER_MODE_LOW_PASS>(delay_out1l); float
  // filter_out1r = svf2r.Process<stmlib::FILTER_MODE_LOW_PASS>(delay_out1r);
  ...
  delay_averager.Add((delay_prev_sample_l * delay_prev_sample_l +
                      delay_prev_sample_r * delay_prev_sample_r) /
                     2);

  if (delay_drywet > 0.99f) {
    delay_drywet = 1.f;
  }
  out1l = sqrt(0.5f * (delay_drywet * 2.0f)) * delay_prev_sample_l +
          sqrt(0.7f * (2.f - (delay_drywet * 2))) * in1l;
  out1r = sqrt(0.5f * (delay_drywet * 2.0f)) * delay_prev_sample_r +
          sqrt(0.7f * (2.f - (delay_drywet * 2))) * in1r;
};
...
void GetSpectraSample(float &outl, float &outr, float inl, float inr) {
  ...
  spectra_output =
      spectra_output; //(sqrt(0.5f * (spectra_dynamics*2.0f))*filtered_out +
                      // sqrt(0.95f * (2.f - (spectra_dynamics*2))) *
                      // spectra_output)*0.5f;
  ...
  outl = (sqrt(0.5f * (spectra_drywet * 2.0f)) * spectra_outl +
          sqrt(0.95f * (2.f - (spectra_drywet * 2))) * inl) *
         0.5f;
  outr = (sqrt(0.5f * (spectra_drywet * 2.0f)) * spectra_outr +
          sqrt(0.95f * (2.f - (spectra_drywet * 2))) * inr) *
         0.5f;
}
...
void GetSpectringsSample(float &outl, float &outr, float inl, float inr) {
  spectra_oscbank.updateFreqAndMagn();
  float rings_1 =
      string_voice[0].Process() *
      (spectrings_accent_amount[0] * attack_lut[spectrings_attack_step[0]] +
       (1 - spectrings_accent_amount[0]));
  float rings_2 =
      string_voice[1].Process() *
      (spectrings_accent_amount[1] * attack_lut[spectrings_attack_step[1]] +
       (1 - spectrings_accent_amount[1]));
  // float rings_3 = string_voice[2].Process()*
  // (spectrings_accent_amount[2]*spectrings_attack_lut[spectrings_attack_step[2]]
  // + (1-spectrings_accent_amount[2]) ); float rings_4 =
  // string_voice[3].Process()*
  // (spectrings_accent_amount[3]*spectrings_attack_lut[spectrings_attack_step[3]]
  // + (1-spectrings_accent_amount[3]) );
